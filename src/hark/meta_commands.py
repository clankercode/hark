...
delivered to the worker agent as a prompt (B009). Matching is deliberately
conservative: only a **whole-utterance** command is treated as meta, so a real
answer like "skip the failing test and continue" is *not* hijacked.
"""
...
def _normalize(text: str) -> str:
    ...
    return _TRAIL_PUNCT.sub("", norm).strip()


def classify_meta_command(text: str) -> str | None:
    """Return the canonical meta-command for a transcript, or ``None``.

    Only a whole-utterance control phrase matches; anything with additional
    substantive content is treated as a normal answer (returns ``None``).
    """
